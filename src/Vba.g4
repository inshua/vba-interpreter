...
moduleDeclarationsElement :
	...
	| moduleOption
	| typeStmt
	| attributeStmt				// sometimes attributeStmt mixed in declares
;
...
constSubStmt : ambiguousIdentifier typeHint? (WS asTypeClause)? WS? EQ WS? constValueExpr;
...
exitStmt : EXIT_DO | EXIT_FOR | EXIT_FUNCTION | EXIT_PROPERTY | EXIT_SUB | EXIT_RULE;
...
ruleStmt :
	(visibility WS)? (STATIC WS)? RULE WS? ambiguousIdentifier typeHint? (ALIAS WS ambiguousIdentifier)? (WS? argList)? (WS? asTypeClause)? (WS? WHEN WS valueStmt)? endOfStatement
	block?
	END_RULE
;
...
goSubStmt : GOSUB WS ambiguousIdentifier;

goToStmt : GOTO WS ambiguousIdentifier;
...
midStmt : MID WS? LPAREN WS? ambiguousIdentifier WS? ',' WS? valueStmt (WS? ',' WS? valueStmt)? WS? RPAREN WS? EQ WS? valueStmt;
...
onGoToStmt : ON WS valueStmt WS GOTO WS ambiguousIdentifier (WS? ',' WS? ambiguousIdentifier)*;

onGoSubStmt : ON WS valueStmt WS GOSUB WS ambiguousIdentifier (WS? ',' WS? ambiguousIdentifier)*;
...
// as type required, type hint not allowed
typeStmt_Element : ambiguousIdentifier (WS? LPAREN (WS? constSubscripts)? WS? RPAREN)? WS asTypeClause endOfStatement;

typeOfIsStmt : TYPEOF WS valueStmt WS IS WS type;
...
variableSubStmt : ambiguousIdentifier (WS? LPAREN WS? (constSubscripts WS?)? RPAREN WS?)? typeHint? (WS asTypeClause)?;
...
implicitCallStmt_InStmt : iCS_startElement iCS_followElement*;
...
memberCall : WS? '.' WS? ambiguousIdentifier;

dictionaryCall : WS? '!' WS? ambiguousIdentifier;
...
argCall : (ambiguousIdentifier WS? ASSIGN WS?)? valueStmt;
...
argDefaultValue : EQ WS? constValueExpr;

subscripts : subscript (WS? ',' WS? subscript)*;

subscript : (valueStmt WS TO WS)? valueStmt;

constSubscripts : constSubscript (WS? ',' WS? constSubscript)*;

constSubscript : (constValueExpr WS TO WS)? constValueExpr;
...
baseType : BOOLEAN | BYTE | DATE | DOUBLE | INTEGER | LONG | SINGLE | CURRENCY | DECIMAL | STRING | VARIANT | OBJECT | IUNKNOWN;
...
fixedSizeString : STRING WS? '*' WS? (literal | ambiguousIdentifier);
